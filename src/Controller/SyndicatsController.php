...
class SyndicatsController extends AdminController
{
    ...
    public function getModel($name = 'Syndicat', $prefix = 'Administrator', $config = ['ignore_request' => true])
    {
    ...
    public function publish()
    {
        ...
        if (empty($cid)) {
            ...
            if ($model->publish($cid, $value)) {
                $this->setMessage(Text::plural('COM_ELUS_N_SYNDICATS_' . ($value ? 'PUBLISHED' : 'UNPUBLISHED'), count($cid)));
            }
        }

        $this->setRedirect(Route::_('index.php?option=com_elus&view=syndicats', false));
    }
}